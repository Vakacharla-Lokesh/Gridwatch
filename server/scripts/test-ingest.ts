...
async function testIngest() {
  ...
  console.log(
    `📦 Batch size: ${SENSOR_COUNT * READINGS_PER_SENSOR} readings\n`,
  );
  ...
  try {
    ...
    if (errorResponse.status === 400) {
      const errorData = (await errorResponse.json()) as {
        error: string;
      ...
      console.log(
        `   Errors: ${errorData.details?.length || 0} validation issues found\n`,
      );
    } else {
